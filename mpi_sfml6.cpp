...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < rows_per_process; i++) {
        for (int j = 0; j < width; j++) {
            ...
            buffer2[index] =  std::abs(buffer1[index]-buffer2[index]);
            buffer2[index+1] = std::abs(buffer1[index+1]-buffer2[index+1]);
            buffer2[index+2] = std::abs(buffer1[index+2]-buffer2[index+2]);
            buffer2[index+3] = 255;
        }
    }
